...
class GOTVisiual(Visual):
    def __init__(self, visual, base):
        #self.visual = visual
        self.image_size = 1024
        self.image_embeds = None
        super().__init__(visual, base)
        self.mm_proj = base.model.model.mm_projector_vary
 
    def load(self):
        self.vision_start_id = self.config.im_start_token
        self.vision_end_id = self.config.im_end_token
        self.image_pad_id = self.config.im_patch_token
        self.llm_config['image_size'] = self.image_size
        self.llm_config['vision_start'] = self.vision_start_id
        self.llm_config['vision_end'] = self.vision_end_id
        self.llm_config['image_pad'] = self.image_pad_id
    ...
    def forward(self, images):
        ...
        image_embeds = self.mm_proj(image_embeds)
        return image_embeds
...
class LlmExporter(torch.nn.Module):
    ...
    def load_pretrained(self, model_path: str):
        ...
        elif "GOT" in model_path:
            from GOT_ocr_2_0 import GOTQwenForCausalLM
    ...
    def load_model(self, model_path):
        ...
        if self.visual is not None:
            ...
            self.visual = GOTVisiual(self.model.model.vision_tower_high, self)
    ...
    def embedding(self, input_ids):
        if self.visual is not None and self.token_len == 0:
            if self.model_type == "GOT":
                input_embeds = self.visual.embed_(input_ids)
            else:
...
def main():
    ...
    parser.add_argument('--path', type=str, default="/data/Learning/GOT-OCR2.0/GOT-OCR-2.0-master/GOT_weights",
    ...
    parser.add_argument('--export', type=str, default="onnx", help='export model to an onnx/mnn model.')
